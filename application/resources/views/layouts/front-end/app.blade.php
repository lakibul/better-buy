...
<head>
    ...
    <link rel="apple-touch-icon" sizes="180x180"
          href="{{asset('application/storage/app/public/company')}}/{{$web_config['fav_icon']}}">
    ...
    <link rel="stylesheet" href="{{asset('assets/front-end')}}/css/nouislider.css">

    <link rel="stylesheet" href="{{asset('assets/front-end')}}/css/style.css">
    <!-- Custom  Styles-->
    <link rel="stylesheet" href="{{asset('assets/front-end')}}/css/custom-style.css">
    <link rel="stylesheet" href="{{asset('assets/front-end')}}/css/custom-responsive.css">
...
<section class="modals-section">
    <!--registration min start-->
    <!-- Button trigger modal -->
    <button type="button" class="btn btn-primary d-none" data-toggle="modal" data-target="#reg-modal">
        registration
    </button>
    ...
    <div class="modal fade" id="reg-modal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" style="max-width: 412px;">
            <div class="modal-content custom-reg-modal">
                ...
                <h5 class="modal-title registration-header mx-auto" id="exampleModalLabel">Register or log in to continue</h5>
                <div class="modal-body custom-modal-body">
                    <div class="reg-btn">
                        <div class="modal-btn-container text-center">
                            <button class="google-btn"><span><i class="fa fa-google" aria-hidden="true"></i></span>Continue with Google</button>
                        </div>
                        <div class="modal-btn-container text-center">
                            <button class="fb-btn"><span><i class="fa fa-facebook" aria-hidden="true"></i></span>Continue with Facebook</button>
                        </div>
                    </div>
                    <div class="modal-form">
                        <form method="post" action="#" id="otp-request-form">
                            @csrf
                            <div>
                                <div><label for="">Mobile Number <span class="mendatory">*</span></label></div>
                                <input type="text" placeholder="Enter Your Mobile Number" name="phone">
                            </div>
                            <div class="mt-2">
                                <div><label for="">Email Address (Optional)</label></div>
                                <input type="text" placeholder="Enter Your email" name="email">
                            </div>
                            <div class="text-center">
                                <button type="button" onclick="otp_request()" id="otp-modal" class="reg-otp-btn" data-toggle="modal" data-target="#log-otp" data-dismiss="modal" aria-label="Close">Get OTP On Mobile</button>
                            </div>
                        </form>
                    </div>
                </div>
    ...
    <!--registration min end-->
    <!--log otp start-->
    <!-- Button trigger modal -->
    <button type="button" class="btn btn-primary d-none" data-toggle="modal" data-target="#log-otp">
        log otp
    </button>
    <!-- Modal -->
    <div class="modal fade" id="log-otp" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" style="max-width: 412px;">
            <div class="modal-content custom-reg-modal">
                <div class="modal-header custom-modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                ...
                <div class="modal-body otp-modal-body">
                    <form method="post" action="#" id="otp-check-form">
                        @csrf
                        ...
                        <div class="otp-placeholder-btn-wrapper d-flex justify-content-between mt-2">
                            <button class="otp-edit-btn" data-toggle="modal" data-target="#reg-modal" data-dismiss="modal" aria-label="Close">Edit Number</button>
                            <button class="otp-resend-btn">Resend OTP</button>
                        </div>
                        <div class="otp-continue">
                            <button class="inactive-btn" type="button" onclick="otp_check()" disabled>Continue</button>
                        </div>
                    </form>
                </div>
            </div>
...
<div class="row">
    <div class="col-12" style="margin-top:10rem;position: fixed;z-index: 9999;">
        <div id="loading" style="display: none;">
            <center>
                <img width="200"
                     src="{{asset('application/storage/app/public/company')}}/{{\App\CPU\Helpers::get_business_settings('loader_gif')}}"
...
<script src="{{asset('assets/front-end')}}/js/nouislider.js"></script>
<script src="{{asset('assets/front-end')}}/js/wNumb.min.js"></script>
...
<script>
    ...
    const otpInputs = document.querySelectorAll(".otp-placeholder input");
    const otpbtn = document.querySelector(".otp-continue button");
    ...
    function handleOtp(e) {
        ...
        if (fieldIndex == otpInputs.length - 1 && isValidInput) {
            otpbtn.classList.remove("inactive-btn");
            otpbtn.classList.add("active-btn");
    ...
    function updateCartQuantity(minimum_order_qty, key, type= null) {
        /* var quantity = $("#cartQuantity" + key).children("option:selected").val(); */
        var quantity;
        if(type !== null){
           if(type === 'plus'){
               quantity = parseInt($("#cartQuantity" + key).val()) + 1;
           }else{
               quantity = parseInt($("#cartQuantity" + key).val())  -1;
           }
        }else{
            quantity = $("#cartQuantity" + key).val();
        }
...
<script>
    let otp_request = () => {
        ...
        let form = document.getElementById("otp-request-form")
        let formData = new FormData(form);

        $.ajax({
            type: 'POST',
            url: "{{route('customer.auth.otp-generate')}}",
            data: formData,
            cache: false,
            contentType: false,
            processData: false,
            success: function (data) {
                ...
                input1.type = "hidden";
                input1.className = "";
                input1.name = "temporary_token";
                input1.value = data.temporary_token;

                document.getElementById("otp-check-form").insertAdjacentElement("beforeend", input)
                document.getElementById("otp-check-form").insertAdjacentElement("beforeend", input1)
...
<script>
    let otp_check = () => {
        ...
        let form = document.getElementById("otp-check-form")
        let formData = new FormData(form);
        $.ajax({
            type: 'POST',
