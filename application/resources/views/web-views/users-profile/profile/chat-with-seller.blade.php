...
<div class="__chat-seller">
    ...
    <div class="container pb-5 mb-2 rtl"
        ...
        <div class="row">
                                    ...
                                    @foreach($unique_shops as $key=>$shop)
                                        <div class="chat_list @if ($key == 0) btn--primary @endif"
                                            ...
                                            <div class="chat_people" id="chat_people">
                                                <div class="chat_img">
                                                    <img onerror="this.src='{{asset('assets/front-end/img/image-place-holder.png')}}'" src="{{ $shop->delivery_man_id ?asset('application/storage/app/public/delivery-man/'.$shop->image) : asset('application/storage/app/public/shop/'.$shop->image)}}"
                                                        class="__rounded-10">
                                                </div>
                                                <div class="chat_ib">
                                                ...
                                                @if ($chat->sent_by_seller? $chat->sent_by_seller : $chat->sent_by_delivery_man)
                                                    <div class="incoming_msg">
                                                        <div class="incoming_msg_img"><img
                                                                src="@if($chat->image == 'def.png'){{asset('application/storage/app/public/'.$chat->image)}} @else {{ $shop->delivery_man_id ?asset('application/storage/app/public/delivery-man/'.$last_chat->delivery_man->image) : asset('application/storage/app/public/shop/'.$last_chat->shop->image)}}
                                                                @endif"
                                                                alt="sunil"></div>
            ...
            @else
                <div class="col-md-8">
                    <h2 class="profile-page-heading">{{ Request::is('chat/seller') ? \App\CPU\translate('chat_with_seller') : \App\CPU\translate('chat_with_delivery-man')}}</h2>
                    <p class="profile-page-head-txt mb-2">This section contains your chat information</p>
                        ...
                        if (data.length != 0) {
                            data.map((element, index) => {
                                ...
                                if (element.sent_by_customer) {
                                    ...
                                    let img_path = element.image == 'def.png' ? `{{ asset('application/storage/app/public/shop') }}/${element.image}` : `{{ (isset($shop->delivery_man_id) && $shop->delivery_man_id) ? asset('application/storage/app/public/delivery-man') : asset('application/storage/app/public/shop') }}/${element.image}`;
