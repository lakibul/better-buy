                    ...
                    <div class="card card-body px-0 h-100">
                        <div class="media align-items-center px-3 gap-3 mb-4">
                            <div class="avatar avatar-sm avatar-circle">
                                <img class="avatar-img" onerror="this.src='{{ asset('assets/front-end/img/image-place-holder.png') }}'" src="{{asset('application/storage/app/public/seller/')}}/{{auth('seller')->user()->image}}" alt="Image Description">
                                        ...
                                        <div class="chat_list p-3 d-flex gap-2 user_{{$chatting->user_id? $chatting->user_id : $chatting->delivery_man_id}} seller-list @if ($key == 0) active @endif"
                                            ...
                                            <div class="chat_people media gap-10" id="chat_people">
                                                <div class="chat_img avatar avatar-sm avatar-circle">
                                                    <img
                                                        @if (Request::is('seller/messages/chat/customer'))
                                                            src="{{ asset('application/storage/app/public/profile/'.$chatting->image) }}"
                                                        @else
                                                            src="{{ asset('application/storage/app/public/delivery-man/'.$chatting->image) }}"
                                                        @endif
                            ...
                            <div class="media align-items-center gap-3">
                                <div class="avatar avatar-sm avatar-circle">
                                    <img class="avatar-img" id="profile_image"
                                         @if (Request::is('seller/messages/chat/customer'))
                                         src="{{ asset('application/storage/app/public/profile/'.$chattings_user[0]->image) }}"
                                         @else
                                         src="{{ asset('application/storage/app/public/delivery-man/'.$chattings_user[0]->image) }}"
                                         @endif
