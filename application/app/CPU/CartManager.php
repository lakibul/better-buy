...
class CartManager
{
    ...
    public static function get_cart_group_ids($request = null): array
    {
    ...
    public static function cart_total_tax()
    {
        $user_cart_id = CartManager::get_cart_group_ids();
        $cart = CartManager::get_cart($user_cart_id);
        $total = 0;
        if (!empty($cart)) {
            foreach ($cart as $item) {
                $product_total_tax = $item['tax'] * $item['quantity'];
                $total += $product_total_tax;
            }
        }
        return $total;
    }

    public static function cart_sub_total()
    {
        $user_cart_id = CartManager::get_cart_group_ids();
        $cart = CartManager::get_cart($user_cart_id);
        $total = 0;
        if (!empty($cart)) {
            foreach ($cart as $item) {
                $product_total_tax = $item['price'] * $item['quantity'];
                $total += $product_total_tax;
            }
        }
        return $total;
